...
<template>
	<div class="navbar">
		...
		" class="navbar-tabs">
			<router-link v-if="authStore.token" :to="`/component`" :class="{
				'router-link-active':
					authStore.currentPath.includes('component'),
			}">
				組件瀏覽平台
			</router-link>
			<router-link :to="`/dashboard${linkQuery.includes('undefined') ? '' : linkQuery
				}`">
				儀表板總覽
			</router-link>
			<router-link :to="`/mapview${linkQuery.includes('undefined') ? '' : linkQuery
				}`">
				地圖交叉比對
			</router-link>
			<router-link :to="`/smartcity${linkQuery.includes('undefined') ? '' : linkQuery
				}`">
				Noob.GPT聊天室
			</router-link>
		</div>
		<div class="navbar-user">
			<button v-if="!(authStore.isMobileDevice && authStore.isNarrowDevice) && !authStore.token"
				class="hide-if-mobile" @click="toggle">
				<span>{{
			...
			<div class="navbar-user-info">
				...
				<ul>
					<li>
						<a href="https://tuic.gov.taipei/documentation" target="_blank" rel="noreferrer">技術文件</a>
					</li>
					<li>
						<button @click="dialogStore.showDialog('contributorsList')">
							專案貢獻者
						</button>
					</li>
				</ul>
			...
			" class="navbar-user-user">
				...
				<ul>
					...
					<li v-else-if="authStore.user.is_admin" class="hide-if-mobile">
